...
from previous.tree import TreeNode
...
class ID3DecisionTree:
    ...
    def boosttrain(self, D_train, wboost, epsilon_thread):
        ID3DecisionTree.boost = wboost
        self.root = self._boost_TreeGenerate(D_train, self.A)

        x_train, y_train = D_train
        y_pred = np.array(self.test(x_train))

        y_pred[y_pred==False] = 0
        y_pred[y_pred==True] = 1
        y_train_ = np.array(y_train)
        y_train_[y_train_==False] = 0
        y_train_[y_train_==True] = 1
        epsilon = wboost.transpose().dot(y_train_^ y_pred)
        # print(epsilon)
        if epsilon <= epsilon_thread or epsilon >= 0.5:
            return wboost, epsilon, 0
        alpha = 0.5 * math.log((1-epsilon)/epsilon)
        wboost_ = wboost
        exp_ = np.exp(-alpha*(y_train_^ y_pred))
        wboost_ = wboost_.reshape(len(wboost_), 1) * exp_.reshape(len(wboost_), 1)
        wboost_ = wboost_/np.sum(wboost_)
        return wboost_, epsilon, alpha
        
    def _boost_TreeGenerate(self, D, A, v=None, depth=0):
        ...
        for v in buf_v:
            ...
            if A[optimal_attribute_idx][1] != "CONTINUOUS":
                if D_v[0]:
                    root.add_child(self._boost_TreeGenerate(D_v, new_A, v,
                                                      depth + 1))
            else:
                if D_v[0]:
                    if v == "pos":
                        root.add_child(
                            self._boost_TreeGenerate(D_v, new_A,
                                               (">", optimal_point),
                                               depth + 1))
                    else:
                        root.add_child(
    ...
    def create_for_train(self, n_bin, index):
        if n_bin == 1:
            return self.X, self.classes, self.X, self.classes
        else:
            folds = self._split_data(n_bin)
            D_train = [[], []]
            D_test = [[], []]
    ...
    def _entropy(D):
        ...
        sum_false = 0
        sum_true = 0
        # print(ID3DecisionTree.boost)
        for i in range(total):
            if y[i]:
                sum_true = sum_true + ID3DecisionTree.boost[i][0]
            else:
                sum_false = sum_false + ID3DecisionTree.boost[i][0]
        temp = []
        if sum_true == 0 and sum_false == 0:
            temp = []
        elif sum_true == 0 or sum_false == 0:
            temp = [1.0]
        else:
            temp = [sum_false/(sum_false+sum_true), sum_true/(sum_false+sum_true)]
